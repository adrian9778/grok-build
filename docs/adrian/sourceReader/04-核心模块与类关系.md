...
Dashboard Store 是 Grok Build 0.4.x 引入的持久化层，负责管理 Agent Dashboard 的工作区状态。

### 5A.1 核心组件

- **`WorkspaceStore`** — 主存储句柄，单写者模式
- **`SessionId`** — 类型安全的会话标识符  
- **`Member`** — 工作区成员（会话）记录
- **`SchemaState`** — 数据库 schema 版本管理
...
pub struct WorkspaceStore {
    conn: rusqlite::Connection,  // 独占 SQLite 连接
    data_version: DataVersion,   // 乐观并发控制
}

// 路径安全保证  
#[derive(Debug, Clone, PartialEq, Eq, Hash)]
pub struct SessionId(String);  // 已验证的单路径组件
```

### 5A.3 关键设计决策

1. **单写者模式**：一个进程只能有一个 `WorkspaceStore` 实例，避免 SQLite 锁竞争
2. **类型安全 ID**：`SessionId` 在创建时验证，防止路径注入攻击
3. **容量限制**：工作区成员数上限 `WORKSPACE_CAPACITY`（当前 200）
4. **Schema 演进**：仅追加式迁移，新旧版本二进制可共存

### 5A.4 事务使用模式

```rust
// 插入成员（自动淘汰最旧的非置顶成员）
let outcome = store.insert_member(member)?;
...
- **主存储**：`crates/codegen/xai-grok-dashboard-store/src/store.rs:58`
- **Schema 定义**：`crates/codegen/xai-grok-dashboard-store/src/schema.rs:15`  
- **类型系统**：`crates/codegen/xai-grok-dashboard-store/src/types.rs:23`
- **错误处理**：`crates/codegen/xai-grok-dashboard-store/src/error.rs:12`
