...
> 本文保留“概念和承诺点”视角。若需要真实文件、函数、trait、channel 和返回类型，请以 [源码符号关系总览](12-源码符号关系总览.md) 为权威入口；若要在 IDE 中逐步跟读，请使用 [关键调用链逐函数精读](13-关键调用链逐函数精读.md)。

```mermaid
flowchart LR
    CONCEPT["09：概念/承诺点"] --> SYMBOL["12：源码符号关系"]
    SYMBOL --> TRACE["13：逐函数执行过程"]
    TRACE --> DETAIL["02–08：领域实现细节"]
```
