...
这套笔记不是按文件树翻译源码。它先给一张六层图（组合根 → 展示 → 应用 → 领域 → 端口 → 适配器），再用一条 Prompt 全路径把 `main`、TUI、`SessionActor`、`ChatState`、Sampler、Tool、Workspace 串起来，最后才按 crate 补细节。

### 核心调用序列（逐步逻辑）

1. 读本篇，确认覆盖口径和 9 问阅读法。
2. 用一个最小例子走完：键盘回车 → `Action::SendPrompt` → `SessionCommand::Prompt` → `SamplingEvent` → 屏幕。
3. 按 `01`–`16` 补分层、端口、失败语义。
4. 重实现时从 `15` 的阶段 0 开始，而不是复制 80 个 crate。

### 易错点与边界条件

- 根 `Cargo.toml` 是生成物；不要靠改它来加依赖。
- `Tool::execute` 才是 runtime 入口；`run` 只是糖。
- 取消不回滚已发生的文件写入。
...
| `docs/adrian/sourceReader1/` | 与本目录同结构的底稿 | 编号对齐；日常阅读以本目录为准 |
| `docs/adrian/sourceReader2/` | 更短的新手地图（原 `docs/sourcecode1/`） | 先建立边界时可用，细节仍以本目录为准 |
