...
> **场景**：grok-build 是 **101 个 member** 的 Cargo workspace，要在 macOS/Linux 上编译、要用 DotSlash 拉 hermetic 的 `protoc`、要生成 protobuf 代码、要把 `rg`/`fd`/`bfs`/`ugrep` 塞进二进制、要跑分层测试，还要对 vendored 与移植的第三方代码做许可证合规。本章讲清「怎么从源码变成可运行的 `xai-grok-pager` 二进制、以及合规边界在哪」。
>
> **阅读说明**：源码基准 `SOURCE_REV = 9bb727ccdff0a793ee73bcde4e2e09cbef6b5387`（仓库根 `SOURCE_REV`；注意它与当前 git HEAD `6c01b90c` 不是同一个 SHA——`SOURCE_REV` 记录的是上游 monorepo 的 commit）。**工具版本**：Rust `1.94.0`（`rust-toolchain.toml:11`）、`ratatui 0.29` / `ratatui-core 0.1`、`tokio` 特性 `full`、`agent-client-protocol 0.10.4`（`Cargo.toml` `[workspace.dependencies]`）。本章一律用「文件 + 符号 + 偏移」定位；行号随版本变化，以当前源码为准。
...
┌────────────────────────────────────────────────────────────────────────────┐
│ ① 构建 (Build)                                                              │
│   Cargo workspace: 101 members                                              │
│     crates/codegen/*  83   crates/common/*  12                              │
│     crates/build/*     1   third_party/*     4   prod/mc/*  1               │
...
|--------|---------|--------------------|-----------|
| 工作区定义 | `members` / `[workspace.dependencies]` | `Cargo.toml`（自动生成，只读） | 101 个 member、版本统一、profile、lint |
| 工具链锁定 | `[toolchain] channel` | `rust-toolchain.toml:11` | 固定 Rust 1.94.0 + rustfmt/clippy |
| 构建期 proto 生成 | `XaiProtoBuilder` / `configure` | `crates/build/xai-proto-build/src/lib.rs` `Struct：XaiProtoBuilder`、`函数：configure` | `.proto` → Rust + pbjson |
| protoc 定位 | `find_protoc` | `xai-proto-build/src/find_protoc.rs` `函数：find_protoc` | `$PROTOC` → `bin/protoc` → `PATH` |
...
① cargo build
   ...
   │  Cargo 读根 Cargo.toml → 展开 101 个 member
   ▼
② 各 crate 的 build.rs 先跑（顺序由依赖图决定）
   ...
   │      ... (8 个 field_attribute，全部是 #[serde(default)])
   ...
   ▼
③ 编译 101 个 crate → 链接出 target/{debug,release}/xai-grok-pager
...
workspace 分层（`Cargo.toml` `members`，共 **101** 项）：

| 目录 | 数量 | 内容 |
|------|-----:|------|
| `crates/codegen/` | 83 | 业务/工具实现（pager、shell、tools、workspace、auth、mcp、telemetry、update…） |
...
`[workspace.package]` 定 `edition = "2024"`、`license = "Apache-2.0"`；`[workspace.dependencies]` 集中版本，其中本章关注的四个是：
...
另有两条 git 覆盖值得注意：`[patch.crates-io] async-openai = { git = "https://github.com/our-forks/async-openai.git", rev = "95b52..." }`，以及 `nucleo = { git = "https://github.com/helix-editor/nucleo.git", rev = "5b74652" }`——即这两个依赖**不走 crates.io**，构建需要能访问 GitHub。

### 步骤 2 — DotSlash 与 protoc 定位

`bin/protoc` **不是二进制，而是一个 DotSlash 清单**（1570 字节，首行 `#!/usr/bin/env dotslash`），声明三平台资产：

| 平台 | 大小 | sha256（前 16 位） | 上游 |
|------|------|-------------------|------|
| `macos-aarch64` | 2 290 929 | `2b8a3403cd097f95` | protoc-29.3-osx-aarch_64.zip |
| `linux-x86_64` | 3 288 836 | `3e866620c5be2766` | protoc-29.3-linux-x86_64.zip |
| `linux-aarch64` | 3 256 816 | `6427349140e01f06` | protoc-29.3-linux-aarch_64.zip |

`find_protoc`（`find_protoc.rs` `函数：find_protoc`）的搜索顺序写得很明确，也是**失败路径与成功路径同等重要**的典型：
...
`check_protoc_good`（同文件 `函数：check_protoc_good`）跑 `protoc --version`，失败时给的提示就是安装指引：**`try \`cargo install dotslash\`;`**。README 也把 DotSlash 列为构建前置条件（`README.md`「Building from source」）。

### 步骤 3 — `XaiProtoBuilder`：protobuf → Rust + pbjson

`crates/build/xai-proto-build/src/lib.rs`：

- `Struct：XaiProtoBuilder` 持有 `builder: tonic_prost_build::Builder` 与一组**本地开关**：`file_descriptor_set_path`、`gen_pbjson`、`pbjson_ignore_unknown_fields`、`pbjson_preserve_proto_field_names`、`pbjson_exclude`、`honor_debug_redact`、`btree_map_paths`。
- `函数：configure`（同文件，lib.rs 尾部）是唯一构造入口，固定三项预设：`compile_well_known_types(true)`、`extern_path(".google.protobuf", "::pbjson_types")`、`extern_path(".google.protobuf.Empty", "()")`、`protoc_arg("--experimental_allow_proto3_optional")`。

builder 方法语义（相对 `Struct：XaiProtoBuilder` 定义行）：

| 方法 | 作用 | 关键实现细节 |
|------|------|-------------|
| `btree_map(paths)` | map 字段用 `BTreeMap` | **同时记进 `btree_map_paths`**，稍后传给 pbjson_build——否则 JSON map key 按 HashMap 顺序序列化，生成文件不是逐字节稳定的 |
| `bytes(paths)` | bytes 字段表示 | 直接转发给 tonic builder |
| `boxed(path)` | 递归类型装箱 | 直接转发 |
| `extern_path(proto, rust)` | 外部 proto → 已有 Rust 路径 | 避免重复生成；配合 `pbjson_exclude` 防 orphan impl |
| `file_descriptor_set_path(path)` | 输出描述符集 | pbjson 生成需要它 |
| `gen_pbjson()` | 生成 pbjson serde 层 | 若调用方没给 fds 路径，**自动建 `tempfile::TempDir`** 并把 fds 写到 `xai-proto-build.pbbin` |
| `pbjson_ignore_unknown_fields()` | 忽略未知字段 | 前向兼容 |
| `pbjson_preserve_proto_field_names()` | JSON 用 snake_case 原名 | 反序列化仍接受 camelCase，因此对已存 camelCase 文档向后兼容 |
| `pbjson_exclude(prefixes)` | 跳过某些类型的 serde 生成 | **按段匹配**：`.pkg.Foo` 不匹配 `.pkg.FooBar` |
| `generate_default_stubs(bool)` | 默认桩 | 转发 |
| `honor_debug_redact()` | 尊重 `debug_redact` 字段选项 | 标注字段在 `Debug` 里打印为 `***`；**该 crate 必须依赖 `veil`** |
| `type_attribute` / `field_attribute` | 注入属性 | 转发给 tonic builder |

`函数：compile_protos` 是真正干活的路径，顺序如下：
...
compile_protos(protos, includes)
 ...
 ├─ Self::emit_rerun_if_changed(...)          ← 自己实现：
...
| crate | `build.rs` 做什么 | 关键符号/常量 |
|-------|------------------|--------------|
| `xai-grok-shell` | 打包 ripgrep | `const RG_VER: &str = "15.0.0"`；env 覆盖 `GROK_SHELL_BUNDLE_RG_PATH`；镜像基址 `GROK_SHELL_RG_DOWNLOAD_BASE`；emit `cargo:rustc-cfg=bundle_rg` 与 `GROK_SHELL_RG_{VER,TARGET,GEN_DIR}` |
| `xai-grok-tools` | 打包 4 个搜索二进制 | `RG_VER=15.0.0`、`FD_VER=10.4.2`（macOS x86_64 钉 `FD_VER_MACOS_X64=10.3.0`）、`BFS_VER=4.1`、`UGREP_VER=7.7.0`；`FD_TARBALL_SHA256` 逐资产钉 sha256；`compress_and_pin` 再 zstd 压缩并 emit `GROK_TOOLS_<NAME>_SHA256` |
| `xai-grok-tools-api` | 跑 proto 生成 + serde 默认值 | 8 处 `field_attribute(..., "#[serde(default)]")`；`compile_protos(&["proto/grok-tools.proto"], &["proto/"])` |
| `xai-grok-pager-bin` | 嵌入版本号 + Windows 链接 | emit `VERSION_WITH_COMMIT={version} ({commit})`（commit 取 12 位）；`rerun-if-changed` 只挂 HEAD / logs/HEAD / 当前分支 ref；Windows msvc 加 `/DELAYLOAD:ProjectedFSLib.dll` + `delayimp.lib` |
| `xai-grok-version` | 只声明环境依赖 | `println!("cargo:rerun-if-env-changed=GROK_VERSION")` |
| `xai-grok-markdown` | 预编译语法集 | 把 `assets/Swift.sublime-syntax` 加载进 `two_face::syntax::extra_newlines()`，`dump_to_uncompressed_file(..., OUT_DIR/syntaxes.bin)`；注释说明原因是 **运行时 `SyntaxSet::build` 会卡住首次 Read 渲染（GB-5513 PTY fold）** |

**失败路径的共性**：搜索二进制打包在**离线环境**下都可覆盖——`GROK_SHELL_BUNDLE_RG_PATH` / `GROK_TOOLS_BUNDLE_{RG,FD,BFS,UGREP}_PATH` 直接给本地二进制；`GROK_SHELL_RG_DOWNLOAD_BASE` 指向内网镜像。Windows 上 ripgrep 自动打包**主动跳过**（上游只发 `.zip`，脚本只会解 `.tar.gz`），靠 `return Ok(())` 在 emit `cfg(bundle_rg)` 之前返回，从而把 `include_bytes!` 编译掉、运行时回落到 PATH 上的 `rg`。`fd` 在非 release 且无覆盖时也跳过（避免拖慢 `cargo check`）。
...
| `admin-cli-dist` | release | `opt-level = "z"`、`lto = true`、`strip = true` | prod-admin-cli 的体积优化（放根清单因为 Cargo 忽略非根 manifest 的 profile） |
| `dev` | — | `panic = "abort"`、`codegen-units = 128`、`debug = "line-tables-only"`、`split-debuginfo = "unpacked"`；`[profile.dev.package."*"] debug = false` | 本地开发；**非 workspace 依赖不产 debug info** |
| `bench` | — | `debug = true` | 基准 |

`.cargo/config.toml` 按 target 给 rustflags（rustflags 非叠加，故每个 target 段重复声明）：

- `x86_64-apple-darwin` / `aarch64-apple-darwin`：`-undefined dynamic_lookup`、`force-unwind-tables=yes`、`link-args=-ObjC`
- `x86_64-unknown-linux-gnu`：`force-unwind-tables=yes`
- `aarch64-unknown-linux-gnu`：`target-cpu=neoverse-v2`、`force-unwind-tables=yes`
- `x86_64-unknown-linux-musl`：另有段（musl 构建用）
...
- `clippy.toml`（**注释明确：clippy 用最近的 clippy.toml 且不合并，所以本文件必须重复根 clippy.toml 的设置**）：`large-error-threshold = 256`、`allow-indexing-slicing-in-tests = true`，以及一组 `disallowed-methods`，每条都带理由：
  ...
  - `std::fs::canonicalize` / `std::path::Path::canonicalize` / `tokio::fs::canonicalize` → Windows 会返回 `\\?\` verbatim 路径，用 `dunce::canonicalize`
  - `std::env::home_dir` / `dirs::home_dir` → grok-home 统一走 `xai-dirs`（`xai_dirs::home_dir`），否则与 Node 的 `os.homedir()`（读 `USERPROFILE`）不一致
  - `std::process::Command::spawn` / `tokio::process::Command::spawn` / `portable_pty::SlavePty::spawn_command` → 未登记的 child 会活过会话，必须用 `xai_tty_utils::ProcessScope::enroll`

`rustfmt.toml` 只有一行：`use_field_init_shorthand = true`。

### 步骤 6 — 测试支撑

规模：**32 个 `tests/` 目录、459 个集成测试 `.rs`、13 个 bench**（`criterion` 0.6）。bench 集中在 pager（`render.rs`/`resize.rs`/`search.rs`/`edit_highlight.rs`）、shell（`fork_copy.rs`/`session_list.rs`/`skills_watcher_startup.rs`/`child_replay_lookup.rs`）、pty-harness（`pty_bench.rs`/`paste_latency.rs`）、markdown、ratatui-inline、fsnotify。

`crates/codegen/xai-grok-test-support/src/` 的公开原语（`lib.rs` 顶部列表即权威清单）：
...
| `GrokStdioClient` | `acp_client.rs` | 驱动真实 `grok agent stdio` 子进程，脚本化 `ClientPolicy`，产出类型化 `TranscriptEntry` |
| `AcpTestClient` | `acp_test_client.rs` | 写 verbatim JSON-RPC 行，读到同 id 响应为止——覆盖类型化客户端造不出的线格式（Foundation `\/` 方法、字符串 UUID id） |
| `MockInferenceServer` | `mock_server.rs` | 桩 `/v1/chat/completions`、`/v1/responses`、`/v1/messages`，带请求日志与会话脚本 |
| `Conversation` / `Tool` / `ObservedFailure` | `conversation.rs` | 每会话的工具调用与回复；`Tool` 从请求里挑名字；失败时用 `ObservedFailure` 记录「mock 对请求做了什么」 |
| `leader::LeaderStdioClient` | `leader.rs` | 驱动 `grok agent --leader stdio`（unix） |
| `TestProcess` | `process.rs` | 自有 detached child 生命周期、进程树清理、有界输出尾巴 |
| `run_headless` | `headless.rs` | 对 mock server 跑 `grok -p` 并捕获输出 |
| `git_workdir` / `grok_binary` | `sandbox.rs` / `env.rs` | 建 git 工作区；解析 grok 二进制（`GROK_BINARY` 或 `cargo_bin`） |
...
| `ResourceSnapshot` | `resources.rs` | RSS/线程/fd 采样，供 soak 测试 |
| `MockOtelServer` / `OtelRecorder` / `otel_decode` | `mock_otel_server.rs` 等 | 收 OTLP/HTTP 日志与指标，解码成类型化事件 |
...
`crates/common/xai-test-utils/src/` 更底层：`env.rs`（环境变量）、`git.rs`（造测试仓库，如 `write_fanout_tree`）、`image.rs`、`runfiles_util.rs`（Bazel runfiles）、`tracing_capture.rs`（捕获 tracing 输出）、`lib.rs`。

另有 PTY 栈：`crates/codegen/ptyctl`（`pty.rs`/`term.rs`/`server.rs`/`session.rs`/`keys.rs`/`wait.rs`/`styled.rs`）+ `ptyctl-cli`（`cli.rs`/`commands/`/`registry.rs`）+ `xai-grok-pager-pty-harness`（含 `benches/pty_bench.rs`、`paste_latency.rs`），构成 TUI 的真实终端端到端验证层。
...
third_party/
  ...
  ├── mermaid-to-svg/     # MIT   + LICENSE + THIRD_PARTY_NOTICES + src/ + Cargo.toml
  ├── dagre_rust/         # Apache-2.0 + LICENCE（英式拼写）
  ├── graphlib_rust/      # Apache-2.0 + LICENCE
  └── ordered_hashmap/    # Apache-2.0 + LICENCE
```
...
vendoring 理由（README 原文要点）：mermaid 布局栈渲染**不可信的模型输出**，内联托管给出完整审计面、钉死精确源码、免疫 crates.io yank；本地补丁与升级清单写在**每个 crate 的 `Cargo.toml` 头部注释**里，作为重新 vendoring 的唯一真源。

`third_party/mermaid-to-svg/Cargo.toml` 的 `VENDORING NOTES` 块列出了必须每次升级重放的改动，其中与本章「构建」相关的几条：

- **不 vendoring** `src/bin/render_mermaid.rs`（CLI 二进制）及其 dev-only 依赖（insta/rust-embed/resvg/roxmltree/image/anyhow/serde*）——`xai-grok-mermaid` 自己用 workspace 的 `resvg` 光栅化，所以这里只当 SVG 生成器。
- **不 vendoring** 所有 `*_tests.rs` 快照/夹具测试与 `reference_svg`/`fixtures`；`[lib] doctest = false`。
- **HERMETIC PATCH**：`src/mermaid_port/mod.rs` 的 `is_enabled()` **无条件返回 `false`**——上游读 `MERMAID_TO_SVG_USE_PORT` 环境变量，而「读环境变量」在不可信输入下是不确定的；被禁用的 port 保留 `#[allow(dead_code)]` 以便未来 diff 最小。
- 丢掉上游声明但 vendored 源码里没引用的 `petgraph`、`regex`；`thiserror` 从上游 `"1.0"` 钉到 workspace 的 `"2"`；跑过 `cargo fmt`。
- **LOCAL REWRITE**：`src/sequence_diagram.rs` 重写（上游 sequence 解析器对大部分标准 Mermaid sequence 语法直接硬失败），补齐 `activate`/`deactivate`、`actor`、`create`/`destroy`、`box…end`、`autonumber`、`par`/`and`、`critical`/`option`、`break`、`rect` 以及完整箭头集——**这条刻意不在源码里标注，本注释是唯一记录，升级时必须重放**。
...
### 步骤 8 — codex / opencode 移植（不在 `third_party/`）
...
crates/codegen/xai-grok-tools/
  ...
  └── src/implementations/
        ...
        │     apply_patch/  {apply,errors,mod,parser,seek_sequence,tool}.rs
        │     grep_files/   {mod,tool}.rs
        │     list_dir/     {mod,tool}.rs
        │     read_file/    {indentation,mod,slice,text_utils,tool}.rs
...
同一目录还声明 release 构建会**嵌入预编译第三方工具二进制**（即步骤 4 的 rg/fd/bfs/ugrep），归属 "Bundled tool binaries" 一节。
...
| `crates/codegen/xai-grok-tools/THIRD_PARTY_NOTICES.md` | codex/opencode 移植 + 打包二进制 | 见步骤 8 |
...
| 名称 | 值 | 来源 |
|------|----|------|
| package | `xai-grok-pager-bin` | `crates/codegen/xai-grok-pager-bin/Cargo.toml` |
| artifact（`[[bin]] name`） | `xai-grok-pager` | 同上；`default-run = "xai-grok-pager"` |
| 安装名 | `grok` | README「official installs ship it as `grok`」 |
| 版本 | `1.0.38`（package）+ `VERSION_WITH_COMMIT` | package version + `pager-bin/build.rs` |
| 运行时版本 | `xai_grok_version::VERSION` / `IS_DEV_BUILD` / `full_version()` | `xai-grok-version/src/lib.rs`：`VERSION` 取 `option_env!("GROK_VERSION")` 否则 `CARGO_PKG_VERSION`；`IS_DEV_BUILD = option_env!("GROK_VERSION").is_none()`；`set_full_version` 由二进制启动时注入 `"<version> (<shortcommit>)"`（幂等，首次生效） |

**为什么版本注入要绕一圈**：`pager-bin/build.rs` 注释说明「只有 release 二进制在自己的 build.rs 里钉 commit 并在启动时注入」，这样**库 crate 不会因为每次 commit 就重编译**——把「随 commit 变化的字符串」限制在最外层 crate。
...
1. **工具链锁定**：`rust-toolchain.toml:11` 固定 Rust 1.94.0；workspace = **101** member，`Cargo.toml` 自动生成只读。
...
5. **测试可控**：`TestSandbox`（env_clear 起步）+ `MockInferenceServer`/`Conversation`（确定性推理）+ `MockOtelServer`（可断言遥测）+ `scaled()`（CI 负载容忍）+ `run_in_local_set`（非 Send 客户端）。
6. **第三方内联 + 移植留痕**：`third_party/` 4 个 crate 源码随仓库走；codex/opencode 移植带 crate 级 `THIRD_PARTY_NOTICES.md` 与 Apache §4(b) 变更通知。
7. **许可证三层可审计**：`LICENSE`（第一方 Apache-2.0）→ `THIRD-PARTY-NOTICES`（全部依赖，18 925 行）→ `third_party/NOTICE` + 各 crate 内许可 + fork/移植的 crate 级 NOTICE。
8. **已知漂移**：`third_party/README.md`/`NOTICE` 仍列 `nfsserve`/`fuser`，但当前树中不存在——以目录实况为准。
