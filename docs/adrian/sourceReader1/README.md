...
本目录是一套面向 **从零重新实现（Re-implement）** 的源码精读文档库。它不是用户手册，也不是设计稿摘要，而是把当前仓库的真实 crate、文件、类型、函数、通道和测试，整理成一套可以照着写出等价系统的工程说明书。
...
7. 与 `docs/design/`、`docs/adrian/sourceReader/`、用户手册的关系。
8. 重实现前最低检查清单。
